...
describe('useMyToolsManager', () => {
  ...
  it('should change filter and refetch tools', async () => {
    ...
    expect(mockedToolsApi.getMyTools).toHaveBeenCalledWith(
  ...
  it('should correctly handle archiving a tool', async () => {
    ...
    mockedToolsApi.getMyTools.mockResolvedValue({ items: [toolToArchive, createMockTool('2', 'draft')], next_cursor: null });
    ...
    // Open dialog
    act(() => {
      result.current.openArchiveDialog(result.current.tools.find(t => t.id === '1')!);
    });
