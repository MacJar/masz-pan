...
begin
  ...
  -- 3) Prepare queries
  v_text_query := '%' || p_q || '%';
  select decrypted_secret into v_openai_api_key from vault.decrypted_secrets where name = 'openai_api_key';

  select extensions.openai_embedding_create(
    api_key := v_openai_api_key,
    input := p_q
  ...
  return query
    ...
    where
      ...
      and (
        t.name ilike v_text_query
        or t.embedding <=> v_query_embedding < 0.8
      )
      and (
